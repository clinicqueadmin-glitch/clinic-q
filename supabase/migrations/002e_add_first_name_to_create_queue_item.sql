-- ============================================================
-- Migration 002e: Add first_name persistence to create_queue_item
-- Date: 2026-09-07
-- Problem:
--   queues.first_name was NULL after creation because the RPC
--   didn't derive or persist it. TV/TTS fell back to full
--   patient_name, violating "first name only" requirement.
--
-- Fix:
--   Derive first_name server-side from p_patient_name using
--   split_part(p_patient_name, ' ', 1) and INSERT it.
--
-- Preserved:
--   - All 20 input parameters (unchanged)
--   - Queue number generation
--   - All other INSERT columns
...
-- ── STEP 2: CREATE with first_name derivation ──
...
BEGIN
  ...
  -- Derive first_name: first word of patient_name
  -- "สมชาย ใจดี" → "สมชาย"
  -- "ทดสอบRPCใหม่" → "ทดสอบRPCใหม่"
  v_first_name := split_part(trim(p_patient_name), ' ', 1);
  ...
  -- INSERT queue record with first_name persisted
  INSERT INTO queues (
    id, clinic_id, number, patient_name, first_name, phone, procedure, procedure_id,
    ...
    v_queue_id, p_clinic_id, v_queue_number, p_patient_name, v_first_name, p_phone,
...
-- ── STEP 4: Verify exactly 1 overload ──
DO $$
DECLARE
  v_count INTEGER;
  v_sig TEXT;
BEGIN
  ...
  ELSE
    RAISE WARNING '⚠️ % overloads still exist', v_count;
  END IF;
END $$;
...
BEGIN
  ...
  IF v_exec_count = 0 THEN
    RAISE EXCEPTION
      'EXECUTE grant for anon/authenticated is missing';
  ELSE
    RAISE NOTICE 'EXECUTE grant verified: % grant(s)', v_exec_count;
  END IF;
END
$$;
